...
class Person(db.Model):
    id = db.Column(db.String(60), primary_key=True, autoincrement=False, default=uuid.uuid4)
    child_ref = db.Column(db.String(20), unique=True , nullable=False)
...
class Address(db.Model):
    ...
    created_on = db.Column(db.DateTime())
    updated_on = db.Column(db.DateTime(), default=datetime.utcnow)
    person_id = db.Column(db.String(60), db.ForeignKey('person.id'), nullable=False)
...
class User(db.Model):
    ...
    passoword = db.Column(db.String(120), nullable=False)
